...
band_perc_area = band_perc * band_area_raster

soma_total_area = 0
for ind in list_indices:
    soma_total_area += band_perc_area[ind[0]][ind[1]]
...
# Empilha todas as matrizes como colunas
lista_matrizes = [band_opunit_id_map,
                  band_lulc_base,
                    band_lulc_end,
                    # band_sec_veg_end,
                    band_ibge_raster,
                    band_condition_map_base,
                    band_condition_map_end,
                    # band_edge_map,
                    # band_fire_map,
                    band_perc_area]
stacked = np.stack(lista_matrizes, axis=-1)  # shape será (x, y, 6)

# Agora convertemos isso em um DataFrame
# Primeiro, "achatamos" a matriz (linha, coluna, valores) em uma lista de linhas
rows, cols = band_lulc_end.shape
dados = stacked.reshape(rows * cols, len(lista_matrizes))
